...
public str make(stringCon(str S)) = S;
...
public str make(syntax::abstractSyntax::id(str Id)) = Id;
...
Figure visNode(CFNode n:statement(loc location, asgStat(PicoId Id, EXP Exp))) = box(text("<Id> := <make(Exp)>"),  vis::Figure::id(getId(n)), gap(8), editIt(n));
